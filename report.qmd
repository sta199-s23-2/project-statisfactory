...
3.  Graph 4: This Graph is showing the logistic regression line for females, with shot and tased at the top, and just shot at the bottom. First, there is not a very high correlation, as we can see, as the logistic regression line does not even show much of its shape. Based on the trends of the graph, if we extended the x axis age by a large margin, we would see the full logistic line. Surprisingly, against what we would have predicted, there seems to be a small positive correlation between greater violence and age. Perhaps that is due to the fact that police officers view older people as less a threat, and start off by using a taser, which if it does not works turns into a shooting. After all, of note is the fact that every piece of data in this dataset is only people who had gotten shot.

4.  Graph 5: As compared to the graph for females, the males graph has an even weaker correlation. It seems that age has barely no difference in whether or not people were just shot, or shot and tasered. However, there is still a weak positive correlation, perhaps due to the explanation given for graph 4 (that older people were more often tasered first, due to being perceived as less of a threat).
